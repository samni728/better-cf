...
[![Version](https://img.shields.io/badge/version-v1.3.1-2563eb)](VERSION)
...
当前 `v1.3.1` 已把 WebUI 拆成执行中心、任务历史、IP 结果和项目配置四个工作区，并新增分阶段 SQLite 搜寻记忆。手动最高优先范围会保留用户输入的种子 IP，先精确复测，再从对应窄网段和父网段深挖；设置手动范围后，每批 100% 候选来自该范围，数据库历史候选和全局随机池本轮暂停。
